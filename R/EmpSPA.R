...
#' @param resid a vector of residuals from a generalized linear regression model in which Cova.mtx is the covariate matrix (only needed when the type of phenotype is neither survival nor binary).
...
#' @param resid a vector of residuals from a generalized linear regression model in which X is the covariate matrix
...
######################################################################################
#'Empirical SaddlePoint Approximation for homozygous option (EmpSPA.homo) implementation of a case-control study (binary phenotype), surival analysis (time-to-event phenotype) or analysis of other types of phenotype for homozygous option.
#'
...
#'Empirical SaddlePoint Approximation for heterozygous option (EmpSPA.hete) implementation of a case-control study (binary phenotype), surival analysis (time-to-event phenotype) or analysis of other types of phenotype for heterozygous option.
#'
#' A fast and accurate method (heterozygous option) for a genome-wide case-control study, survival analysis or analysis of other traits on a large-scale dataset.
#' @param obj.null an R object returned from function EmpSPA_Null_Model().
#' @param Geno.mtx a numeric genotype matrix with each row as an individual and each column as a genetic variant.
#'                 Column names of genetic variations and row names of subject IDs are required.
#'                 Missing genotypes should be coded as NA. Both hard-called and imputed genotype data are supported.
...
#' @param missing.cutoff a numeric value (default: 0.15) to specify the cutoff of the missing rates.
#'                       Any variant with missing rate higher than this cutoff will be excluded from the analysis.
#' @details To run EmpSPA.hete, the following two steps are required:
#' \itemize{
#'   \item Step 1: Use function EmpSPA_Null_Model() to fit a null logistic regression model for binary phenotype, a null Cox model for time-to-event phenotype, or input residuals from a null generalized linear model to calculate the empirical CGF
#'   \item Step 2: Use function EmpSPA.hete() to calculate p value for each genetic variant.
#' }
#'
#' EmpSPA.hete calculate p values with two empirical saddlepoint approximation methods for heterozygous option (EmpSPA and EmpSPA2) and two empirical normal distribution approximation methods for heterozygous option (Normal1 and Normal2).
#' Generally speaking, empirical saddlepoint approximation methods (EmpSPA and EmpSPA2) are more accurate than, but a little slower than, the empirical normal approximation (Normal1 and Normal2) and the empirical saddlepoint approxiamtions for homozygous option.
#' When the score statistic is very close to 0 (i.e. p-values are not small), empirical saddlepoint approximation (EmpSPA and EmpSPA2) may be numerically unstable.
#' To solve this problem, we replace the wrong p value from EmpSPA and EmpSPA2 by the p values from Normal1 and Normal2, respectively.
#'
#' To calibrate the score statistics for Cox PH model or logistic model, EmpSPA.hete uses martingale residuals which are calculated via R package survival or residuals from glm(). As for the score statistics for other types of phenotype, residuals from a null model are needed.
#' All extentions (such as strata, ties, left-censoring) supported by package survival could also be used in EmpSPA.hete.
#' Time-varying covariates are also supported by splitting each subject into several observations.
#'
#' Sometimes, the order of subjects between phenotype data and genotype data are different, which could lead to some errors.
#' To avoid that, we ask users to specify the IDs of both phenotype data (pIDs) and genotype data (gIDs) when fitting the null model.
...
#' \item{MAF}{Minor allele frequencies}
#' \item{missing.rate}{Missing rates}
#' \item{p.value.empspa}{p value from a empirical saddlepoint approximation (EmpSPA).}
#' \item{p.value.norm1}{p value from a empirical normal distribution approximation (Normal1).}
#' \item{p.value.norm2}{p value from a empirical normal distribution approximation (Normal2).}
#' \item{p.value.empspa2}{p value (recommanded) from a empirical saddlepoint approximation (EmpSPA2).}
#' \item{Stat}{score statistics}
#' \item{Var1}{estimated variances(from Normal1) of the score statistics}
#' \item{z1}{z values(using Var1) corresponding to the score statistics}
#' \item{Var2}{estimated variances(from Normal2) of the score statistics}
#' \item{z2}{z values(using Var2) corresponding to the score statistics}
#' @examples
#'
#' # example 1  time-to-event phenotype (heterozygous)
#' # Simulation phenotype and genotype
#' N = 10000
#' nSNP = 1000
#' MAF = 0.1
...
#' head(survival.res)
#'
#' # example 2  binary phenotype (heterozygous)
#' # Simulation phenotype and genotype
#' N = 10000
#' nSNP = 1000
#' MAF = 0.1
...
#' head(binary.res)
#'
#'# example 3 continuous phenotype (traits = "others") (heterozygous)
#'# Simulation phenotype and genotype
#'N = 10000
#'nSNP = 1000
#'MAF = 0.3
...
#' Empirical SaddlePoint Approximation implementation for homozygous option of logistic regression case-control analysis, Cox regression surival analysis or analysis of other traits (One-SNP-version homozygous option)
#'
#' One-SNP-version EmpSPA.homo function. This function is to facilitate users that prefer reading and analyzing genotype line-by-line.
                               ...
                               G.model = "Add")
{
  ...
  re1 = GetProb_SPA(obj.null, G1N1, G1N0, N1set, N0, abs(S), lower.tail = FALSE) # EmpSPA.homo p value and a
  re2 = GetProb_SPA(obj.null, G1N1, G1N0, N1set, N0, -abs(S), lower.tail = TRUE) # EmpSPA.homo p value and a
  a1 = re1[2]
  a2 = re2[2]

  pval1 = re1[1] # EmpSPA.homo
  pval2 = re2[1] # EmpSPA.homo
  pval3 = pnorm(abs(z1), lower.tail = FALSE) # Normal1.homo
  pval4 = pnorm(-abs(z1), lower.tail = TRUE) # Normal1.homo
  pval5 = pnorm(abs(z2), lower.tail = FALSE) # Normal2.homo
  pval6 = pnorm(-abs(z2), lower.tail = TRUE) # Normal2.homo
  pval7 = pnorm(a1 * sqrt(S.var1/S.var2), lower.tail = FALSE) # EmpSPA2.homo
  pval8 = pnorm(a2 * sqrt(S.var1/S.var2), lower.tail = TRUE) # EmpSPA2.homo
  pval.empspa = pval1 + pval2
  pval.norm1 = pval3 + pval4
  pval.norm2 = pval5 + pval6
  pval.empspa2 = pval7 + pval8

  pval = c(pval.empspa,  pval.norm1, pval.norm2, pval.empspa2) # 4 elements: element 1 is from EmpSPA.homo, element 2 is from Normal1.homo, element 3 is from Normal2.homo, element 4 is from EmpSPA2.homo

  return(c(MAF, missing.rate, pval, S, S.var1, z1, S.var2, z2))
}

#' Empirical SaddlePoint Approximation for heterozygous option implementation of logistic regression case-control analysis, Cox regression surival analysis or analysis of other traits (One-SNP-version for heterozygous option)
#'
...
#' @param g a numeric genotype vector. Missing genotypes should be coded as NA. Both hard-called and imputed genotype data are supported.
#' @param others the same as function EmpSPA.homo. NOTE that we do not check subject order in this one-snp-version !!!
